...
public class KafkaConsumerConfig {
    ...
    public ConsumerFactory<String, OrderEvent> consumerFactory(ObjectMapper mapper) {
        JsonDeserializer<OrderEvent> deserializer = new JsonDeserializer<>(OrderEvent.class, mapper);
...
* **KafkaTemplate / KafkaProducer 추상화**
  `KafkaTemplate`은 실제 Kafka 클라이언트인 `KafkaProducer`를 감싼 스프링 추상화로, 메시지 전송을 간결하게 처리하고 구성 가능한 콜백/리스너를 붙일 수 있게 만든다.


* **신뢰성 설정**

  * `acks=all`: 모든 ISR(in-sync replica)으로부터 확인을 받아야 전송을 성공으로 간주하므로 데이터 손실 위험을 줄인다.
  * `retries`: 일시적 실패 시 재전송 시도. 재전송 시 중복 전송 가능성 때문에 `enable.idempotence=true`를 함께 쓰면 동일한 키/파티션에 대해 중복 없이 전송 보장이 강화된다.
...
* **직렬화**
  ...
  * 복잡한 도메인 객체(예: record 또는 DTO)는 Jackson이 역직렬화할 수 있는 구조여야 하며, 타임타입(`LocalDateTime` 등)은 직렬화 포맷/모듈 등록이 올바른지 확인해야 한다.
    - `JacksonConfig` 에 별도의 `ObjectMapper`를 두고 producer와 consumer 에 동일하게 주입하는 방식으로 해결했다.


* **전송 결과 처리**

  * 비동기 전송 결과는 `CompletableFuture` 또는 `ListenableFuture`(구 버전) API로 받는다. 성공/실패 콜백을 붙여 로깅, 재시도 정책, 알림 등을 구현할 수 있다.
...
* **Consumer 측**: 수동 커밋을 쓰면 처리 완료를 확실히 한 뒤 오프셋을 옮겨 “정확한 처리 제어”가 가능하지만, 실패하면 재처리될 수 있다.
